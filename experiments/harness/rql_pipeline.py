...
Usage (from repo root):
  tooling/.venv/bin/python experiments/harness/rql_pipeline.py \\
      --inputs examples/toy --profiles qdrant,elasticsearch,pgvector \\
      --out-dir experiments/results/e2e --validate --run-id smoke

  tooling/.venv/bin/python experiments/harness/rql_pipeline.py run-batch \\
      --inputs examples/toy --validate
"""
...
def run_batch(args: argparse.Namespace) -> int:
    ...
    if not out_base.is_absolute():
        out_base = (REPO / out_base).resolve()
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--out-dir",
        type=Path,
        default=REPO / "experiments" / "results" / "e2e",
