...
Dhulipala et al.~\citep{jayaram2024muvera} reduce Chamfer/MaxSim multi-vector search to single-vector MIPS via Fixed Dimensional Encodings, then Chamfer rerank---contrasting PLAID's multi-stage centroid pipeline~\citep{santhanam2022plaid}.
Pass~1--5 notes (journal 0013; \texttt{knowledge/reads/muvera-2405.19504/}) treat the \textbf{FDE reduction + MIPS + MaxSim rerank mechanism} as \established{} literature; RQL rewrite packaging and unreproduced BEIR/PLAID numbers remain \hypothesis{} / \provisional{}.
PLAID itself is cite-only pending a dedicated multimodal pass.
