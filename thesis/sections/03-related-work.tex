...
\noindent\textit{We separate \established{} mechanisms and survey structure from \hypothesis{} RQL packaging and from \authoronly{} unreproduced empirics.
Skim the subsection titles if you already know FANNS/RRF/ColBERT; the multimodal notes live under \texttt{journal/} and \texttt{knowledge/reads/}.}
...
A \textbf{docs-only adapter-surface survey} (2026-09-16; journal 0020; \texttt{docs/09-vendor-api-matrix.md}) tabulates filter DSLs, filter$+$ANN composition (PRE/POST/iterative/UNKNOWN), hybrid/BM25$+$dense, RRF and weighted fusion primitives, multi-vector/late interaction, query surfaces, and EXPLAIN/plan visibility for Qdrant, Elasticsearch/OpenSearch, Weaviate, Milvus, and pgvector (optional Pinecone/Redis) \established{} as a \emph{documentation survey}---not a live bake-off.
None of these vendors offers a cross-vendor ANSI-SQL analogue for multi-stage RAG plans \established{} (landscape notes + matrix).
The hypothesis that RQL \texttt{LogicalPlan} compiles through negotiated shims onto these surfaces is stated in \S\ref{sec:compile-hypothesis}.
...
\paragraph{HyDE (multimodal re-read).}
Gao et al.~\citep{gao2023hyde} factorize fully zero-shot dense retrieval \emph{without relevance labels} as Hypothetical Document Embeddings: an instruction-following LM generates one or more hypothetical documents from query+instruction; an unsupervised contrastive encoder (Contriever/mContriever) embeds them; real documents are retrieved by MIPS in document space (Eqs.~(4)--(8); Fig.~1).
Query--document similarity is not explicitly trained; the dense bottleneck is argued to filter hallucinations.
Pass~1--5 notes (journal 0032; \texttt{knowledge/reads/hyde-2212.10496/}) treat the \textbf{hyp-doc $\rightarrow$ encode $\rightarrow$ doc--doc retrieve mechanism} as \established{}; author DL19/20, BEIR, and Mr.TyDi tables remain unreproduced \provisional{}.
RQL's \texttt{REWRITE HYDE} / $\mathrm{Rewrite}$ surface is \hypothesis{} packaging of that pipeline (token-costed plan node feeding dense search), not a claim of reproduced IR gains.

\paragraph{MMR (multimodal re-read).}
Carbonell and Goldstein~\citep{carbonell1998mmr} define Maximal Marginal Relevance as greedy selection
$\arg\max_{D_i\in R\setminus S}[\lambda\,\mathrm{Sim}_1(D_i,Q)-(1-\lambda)\max_{D_j\in S}\mathrm{Sim}_2(D_i,D_j)]$
with $\lambda\in[0,1]$ trading relevance ($\lambda{=}1$) against novelty/diversity ($\lambda{=}0$).
Pass~1--5 notes (journal 0033; \texttt{knowledge/reads/mmr-carbonell-sigir98/}) treat the \textbf{MMR formula + $\lambda$ extremes} as \established{}; author Table~1 sentence precision and SUMMAC F-scores remain unreproduced \provisional{}.
RQL's \texttt{DIVERSIFY MMR} / $\mathrm{Diversify}_{mmr}$ surface is \hypothesis{} packaging of that formula (post-retrieve diversity node)---distinct from multi-channel fusion---and not a claim of reproduced summary/IR gains.
...
\label{sec:rw-substrait-calcite}
Substrait~\citep{substrait} \established{} a cross-language \emph{serialized plan} IR (plans not SQL text), with extension relations/functions and only a \emph{conventional} logical/physical split (docs pass: journal 0025; OKF \texttt{knowledge/reads/substrait-spec-2026-09/}).
Apache Calcite~\citep{begoli2018calcite} \established{} an embeddable algebra+optimizer over heterogeneous adapters, modeling physical properties as traits/calling conventions and citing Volcano/Cascades lineage~\citep{graefe1995cascades} (multimodal pass: journal 0025; OKF \texttt{knowledge/reads/calcite-begoli-sigmod18/}).
RQL steals portable-plan and adapter doctrine as \hypothesis{} adjacency---it does \textbf{not} claim to implement Substrait or embed Calcite (\S\ref{sec:substrait-calcite-adjacency}).
BigDAWG-style polystores~\citep{duggan2015bigdawg} motivate islands + shims \provisional{}.
BlinkDB~\citep{agarwal2013blinkdb} \established{} the AQP idea of \emph{dual declarative contracts}---either an error+confidence bound (\texttt{ERROR WITHIN $\epsilon$ AT CONFIDENCE $C$}) or a response-time bound (\texttt{WITHIN $T$})---plus runtime Error-Latency Profile (ELP) sample-size selection over multi-resolution stratified samples (Pass~1--5: journal~0031; OKF \texttt{knowledge/reads/blinkdb-eurosys13/}).
Author multi-TB latency/error tables remain unreproduced \provisional{}.
RQL's \texttt{RECALL TARGET} / \texttt{LATENCY} surface is \hypothesis{} packaging inspired by that contract+ELP doctrine; it does \textbf{not} claim BlinkDB aggregate confidence intervals transfer to ANN recall@$k$ (\S\ref{sec:budgets}).
